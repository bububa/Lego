...
class ImportKeywords(YAMLObject, Base):
    ...
    def __repr__(self):
        return "%s(host=%r, port=%r, user=%r, passwd=%r, db=%r, query=%r)" % (self.__class__.__name__, self.host, self.port, self.user, self.passwd, self.db, self.query)
    ...
    def run(self):
        ...
        try:
            ...
            if debug: raise CollectiveError('Fail to get keywords! %r'%Traceback())
        if not keywords:
            if debug: raise CollectiveError('No keywords!')
        self.output = 0
...
class ImportSearches(YAMLObject, Base):
    yaml_tag = u'!ImportSearches'
    def __init__(self, host, port, user, passwd, db, query, siteid, strict=None, debug=None):
        self.host = host
        self.port = port
        self.user = user
        self.passwd = passwd
    ...
    def run(self):
        ...
        try:
            searches = self.readdb(query, True)
        except:
            if debug: raise CollectiveError('Fail to get searches! %r'%Traceback())
        if not docs:
        ...
        while searches:
            ...
            for search in iter(searches):
                threadPool.run(self.update, callback=self.result_collection, search=search, ks=ks, siteid=siteid, strict=strict, debug=debug)
                #self.update(doc, siteid, strict, debug)
                max_id = search['ori_id']
            threadPool.killAllWorkers()
            self.save_searches(siteid, debug)
            query = self.chunk_query(max_id, chunk)
            searches = self.readdb(query, False)
        return self.output
    ...
    def update(self, search, ks, siteid, strict, debug):
        if siteid: 
            search['_id'] = '%d:%d'%(search['_id'], siteid)
        else:
            search['_id'] = str(search['_id'])
        if debug: print 'ImportSearches: search=%r'%search['_id']
        for k, v in search.items():
            if isinstance(v, str): search[k] = v.decode('utf-8')
        return self.tf(search, ks, siteid, strict)
    ...
    def save_search(self, siteid, debug):
        for res in self.output:
            ...
            if siteid: searchObj['siteid'] = siteid
            searchObj['keywords'] = keywords
            searchObj['hits'] = hits
            searchObj['tfs'] = tfs
            searchObj['content'] = search['content']
            searchObj['updated_at'] = datetime.utcnow()
            searchObj.save()
            if debug: print 'Saved: search=%r'%search['_id']
        self.output = []
        
    def tf(self, search, ks, siteid, strict):
        keywords = []
        hits = []
        tfs = []
        total = 0
        search_content = search['content'].lower()
        ...
        for keyword in ks:
            ...
            if not strict:
                try:
                    ...
                    kk = name.split()
                h = sorted([search_content.count(k.strip()) for k in kk if k.strip() and len(k.strip()) > 1 and k.strip() not in black_list])
                if not h: continue
                hit = h[0]
            else:
                hit = search_content.count(name)
            if not hit: continue
            total += hit
            keywords.append(keyword['_id'])
            hits.append(hit)
        return search, keywords, hits, [1.0 * hit/total for hit in hits]
...
class SIDF(YAMLObject, Base):
    ...
    def run(self):
        ...
        for i in xrange(0, total_workers, max_chunk):
            ...
            threadPool = ThreadPool(len(ks))
            for keyword in ks:
                threadPool.run(self.sidf, callback=self.update, keyword=keyword, Dd=Ddocs, debug=debug)
            threadPool.killAllWorkers()
        return self.output
    ...
    def sidf(self, keyword, Dd, debug):
        ...
        Dk += 1
        res = log(Dd/Dk)
        if debug: print '!SIDF: keyword:%s, %r, %f'%(keyword['_id'], keyword['name'], res)
        return keyword, Dk, res

    def update(self, response):
        keyword, searches, sidf = response
        keyword['sidf'] = sidf
...
class COEF(YAMLObject, Base):
    yaml_tag = u'!COEF'
    def __init__(self, siteid=None, running=None, max_relations=None, with_searches=None, debug=None):
        self.siteid = siteid
    ...
    def run(self):
        ...
        if hasattr(self,'with_searches') and self.with_searches: with_searches = self.with_searches
        ...
        for i in xrange(0, total_workers, max_chunk):
            ...
            for keyword in iter(ks):
                threadPool.run(self.coef, callback=self.update, keyword=keyword, with_searches=with_searches, debug=debug)
    ...
    def coef(self, keyword, with_searches, debug):
        ...
        for i in xrange(0, total_workers, max_chunk):
            ...
            threadPool = ThreadPool(len(ks))
            for k in iter(ks):
                threadPool.run(self.rank, callback=self.result_collection, keyword=keyword, k=k, with_searches=with_searches, debug=debug)
    ...
    def rank(self, keyword, k, with_searches, debug):
        keyword_id, Dab = k
        while True:
            try:
        ...
        try:
            if with_searches:
                cf = 1.0 * Dab/(keyword['hits'] + rk['hits'] - Dab)/rk['sidf']
            else:
